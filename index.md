...
Large examples are downloaded and converted only when explicitly
requested. The prepared Seurat and CRB files are cached outside the
package and reused on later runs:
...
Both modes require Seurat, SeuratObject, and BPCells. Seurat’s sketch
workflow creates the UMAP and clusters. The sources are public 10x
Genomics datasets; no large data file is stored in this Git repository.
Allow roughly 1 GB of cache space for 50K and 11 GB for 1M after
preparation.
