...
.readBundleBackend <- function(crb_path) {
  ...
  core_methods <- c("getVersion", "getMetaData", "getCellNames")
...
#' under \code{data/}. Raw \code{.crb}, H5, and BPCells artifacts are not
#' registered as HTTP resources. Spatial background images are the deliberate
...
#' deployment system's responsibility. Inputs and target ancestors must not be
...
createShinyApp <- function(
  ...
  crb_targets <- paste0("data/", basename(cerebro_data))
  copy_plan <- list()
  ...
  ## Cerebro data and external backends remain under the private data/ tree.
