...
![The interactive Cerebro app your processed data becomes &mdash; the dimensional-reduction view.](figures/app_projection.jpeg)

> **Demo data & source.** This walkthrough uses Seurat's built-in `pbmc_raw`
> 80-cell example table (`system.file("extdata", "pbmc_raw.txt", package = "Seurat")`).
> The larger, real public datasets shipped in the app are documented &mdash; with
> acquire commands and full build code &mdash; in
> [Demo datasets: sources &amp; processing](demo_datasets.html).

# Overview

The `CerebroNexus` package has two main purposes: (1) Give access to the Cerebro user interface, and (2) provide a set of functions to pre-process and export scRNA-seq data for visualization in Cerebro.
...
Key features that can be accessed through Cerebro:
...
Before starting the workflow, we need to install `CerebroNexus`, as well as the [`Seurat`](https://satijalab.org/seurat/), [`monocle`](https://cole-trapnell-lab.github.io/monocle-release/) and [`SingleR`](https://www.bioconductor.org/packages/release/bioc/html/SingleR.html) packages, which are not installed as dependencies of CerebroNexus because they are only necessary if you want/need to pre-process your scRNA-seq data.
...
if ( 'CerebroNexus' %in% installed.packages() == FALSE ) install('mihem/CerebroNexus')
...
seurat@misc$technical_info <- list(
  'CerebroNexus' = utils::packageVersion('CerebroNexus'),
  'Seurat' = utils::packageVersion('Seurat'),
...
Now we use a set of CerebroNexus functions to get more information about the data set and the contained clusters and cell types.
...
We extract the trajectory from the generated Monocle object with the `extractMonocleTrajectory()` function of CerebroNexus and attach it to our Seurat object.
...
Finally, we use the `exportFromSeurat()` function of CerebroNexus to export our Seurat object to a `.crb` file which can be loaded into Cerebro.
