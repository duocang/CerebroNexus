...
![The shipped Omnibus demo with expression, spatial, Trekker and TCR panels. The control bar selects the colour field, embeddings and spatial sections; an orange outline marks the active panel.](img/linked-views-overview.png)

# Run the shipped demo

The following commands work from a source checkout. They rebuild the synthetic
Omnibus example deterministically, then start the application with its bundled
demos. The build requires the package's normal development dependencies plus
`Matrix` and `base64enc`.
...
#   all gates passed.
#   PUBLISHED inst/extdata/examples/demo_omnibus.crb (... MB)
...
In the **Select dataset** control choose **Omnibus (all modalities)**, then
open **Linked views**. The initial workspace contains an expression embedding,
a spatial section, Trekker physical coordinates and a TCR clone panel. This is
the same dataset used for the screenshot above.
...
```{r}
omnibus <- "inst/extdata/examples/demo_omnibus.crb"
createShinyApp(
  cerebro_data = c("Omnibus (all modalities)" = omnibus),
