...
Before starting the workflow, install `CerebroNexus`, as well as the [`scran`](https://bioconductor.org/packages/release/bioc/html/scran.html), [`scater`](https://bioconductor.org/packages/3.11/bioc/html/scater.html) and [`SingleR`](https://www.bioconductor.org/packages/release/bioc/html/SingleR.html) packages. The latter packages are only needed for pre-processing scRNA-seq data.
...
Finally, we use CerebroNexus `exportFromSCE()` to export our SCE object to a `.crb` file which can be loaded into CerebroNexus.
