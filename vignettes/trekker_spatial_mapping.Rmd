...
![The Trekker tab: physical space (Location CSV, µm) beside the whole-transcriptome UMAP; a selection in one pane highlights the same nuclei in the other.](figures/app_trekker.jpeg)

> **Demo data & source.** Shown on **`Mouse brain (Trekker)`** (`demo_trekker.crb`),
> a down-sampled subset of the Curio/Takara `Mouse_Brain_TrekkerU_C` example bundle
> (access-gated registration). Provenance and full build code:
> [Full pipeline: Mouse brain (Trekker)](demo_dataset_trekker.html).
...
from the **Location CSV only** and ships that as the sole source; the other two
orientations are not offered as an in-app switch — they're documented here,
in this table, so the discrepancy is visible rather than silently baked into
the demo.
