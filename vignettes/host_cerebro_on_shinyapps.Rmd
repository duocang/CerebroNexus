...
To upload CerebroNexus to shinyapps.io, install `rsconnect` and CerebroNexus.
...
Use `createShinyApp()` to build a deployable application directory. The example
below uses the data bundled with CerebroNexus; replace `crb` with your own `.crb`
path and choose a label that users will see in the application.
...
The generated directory contains `app.R`, the Viewer runtime, and the private
data files needed by the deployed application. See
`vignette("create_a_self_contained_shiny_app")` for deployment options and
external expression-backend requirements.
