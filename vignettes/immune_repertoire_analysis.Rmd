...
Because attaching `scRepertoire` pulls in a large dependency tree, CerebroNexus
loads it **on demand** — not at app startup. After the first page renders,
`scRepertoire` is prewarmed in the background, so by the time you open a
repertoire plot it is usually already loaded and renders immediately. (Only if
you jump to a repertoire statistic within the first few seconds might you wait
briefly while it finishes loading.) Repertoire figures are still computed by
`scRepertoire` — only the timing of the load changed.
