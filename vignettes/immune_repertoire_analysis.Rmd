...
loads it **on demand** — only when you first draw a repertoire plot, not at app
startup. The first plot in a session therefore takes a moment longer while the
package loads; everything after is immediate. Repertoire figures are still
computed by `scRepertoire` — only the timing of the load changed.
