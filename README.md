...
> **Large-data benchmark status.** A reproducible real-data harness is being reviewed as repository-only infrastructure under [`tools/bench/`](tools/bench/README.md). It adds independent-process repetitions, correctness fingerprints, exact code and environment provenance, and host resource checks without shipping the large-data workflow or its dependencies in the R package. No publication-profile run has been completed for the current protocol, so its archived pilot output is exploratory and does not replace the PBMC fixture results above. See the [methodology](tools/bench/METHODOLOGY.md) and [result-reading guide](tools/bench/RESULTS.md) before interpreting any run.
...
`result_dir` only after the complete build succeeds, so a missing backend
cannot destroy a working deployment. With `overwrite = FALSE`, `result_dir`
