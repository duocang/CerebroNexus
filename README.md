...
> **Large-data benchmark status.** A new reproducible real-data harness is being reviewed under [`tests/bench/`](tests/bench/README.md). It adds independent-process repetitions, correctness fingerprints, provenance, and host resource checks. No publication-profile run has been completed for the current branch, so its archived pilot output is exploratory and does not replace the PBMC fixture results above. See the [methodology](tests/bench/METHODOLOGY.md) and [result-reading guide](tests/bench/RESULTS.md) before interpreting any run.
