...
profile <- manifest_values[["profile"]]
if (is.null(profile) || is.na(profile) || !nzchar(profile)) {
  stop("run manifest has no benchmark profile", call. = FALSE)
}
...
bench_require_safe_plan(assessment, profile = profile)
