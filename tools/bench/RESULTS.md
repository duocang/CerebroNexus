...
Open `result/CURRENT`, then read `result/runs/<run-id>/summary.md`. Only a validated immutable `publication` run may support numerical claims in the user-facing backend guide. `quick` is exploratory, `standard` is local review evidence, and `stress` may describe an observed host boundary.
...
| `backend ready s` | load plus attach; server-side readiness proxy | excludes Shiny and browser work |
| `first gene ready s` | load, attach, and the first getter call | excludes Shiny and browser work |
...
| `run_manifest.csv` | exact code SHA, dependency-environment blob, R/package versions, CPU, RAM, and OS provenance |
