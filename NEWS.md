...
- **`createShinyApp()` now follows each `.crb` backend descriptor.** H5 files
  ...
  Validation reads locked R6 structure without invoking
  serialized getter, active or lazy bindings; `.crb`/`.rds` inputs nevertheless
  remain trusted serialized R objects, not sandboxed content.
...
- **Bundle publication now has an explicit private/public boundary and recovery
  protocol.** Raw `.crb`, H5 and BPCells artifacts stay in the non-HTTP `data/`
  tree; only files explicitly supplied through `spatial_images` enter the
  public `spatial-assets/` resource path and must be trusted browser-safe
  images. Each canonical target has one atomic build lock covering
