...
##
## Output goes to data-raw/builder_fixtures/ and is gitignored -- it is
...
add_section <- function(obj, name, cells, origin = c(0, 0), span = c(100, 80)) {
  coords <- data.frame(
...
save_as <- function(obj, path) {
  ...
  say(sprintf("%-26s %6.1f MB", basename(path), file.size(path) / 1048576))
  invisible(path)
}
...
o <- add_section(o, "pixels", colnames(o), origin = c(0, 0), span = c(600, 400))
...
save_as(
  make_object(15000, n_genes = 400, samples = c("donorA", "donorB", "donorC")),
