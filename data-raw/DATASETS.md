...
| Field | Meaning |
|-------|---------|
| **type** | `immune_repertoire` \| `spatial` \| `trekker` \| `trajectory` |
...
Real Trekker output sampled from one official example bundle.
Built by `data-raw/build_trekker_official_demo.R` (see [`trekker.md`](trekker.md) for the sampling contract).

### demo_trekker_mouse_brain_u_c.crb
- **type**: trekker
- **technology**: TrekkerU C
- **dropdown label**: `Mouse brain (TrekkerU C)`
- **organism / tissue**: mouse (mm) / brain
- **source**: official `Mouse_Brain_TrekkerU_C` example supplied in the Trekker official data bundle; the original object contains 7,420 confidently positioned nuclei.
- **acquire**: obtain the official bundle, then set `CEREBRONEXUS_TREKKER_DATA_DIR` to its `trekker-official/` directory.
- **object type**: Cerebro export derived from the official Seurat 5 object, with schema-v1 Trekker companion data.
- **sampling**: `set.seed(42)`; up to 40 nuclei from each official `seurat_cluster` (706 total in the current source); 100 RNA genes selected from the top three official markers per cluster, the top 30 official Moran-ranked genes, then top-expressed genes. Location rows and marker/Moran tables are restricted to the selected nuclei/genes; run-level summary metrics remain unchanged.
- **cell-type field**: `seurat_clusters` (official clusters 0–17)
- **embedded image**: none; the official `SlideSeq` image is a coordinate container without a tissue raster, and the Trekker Location coordinates are retained separately. For UI coverage only, sampled nuclei are ordered spatially within each official cluster and assigned alternately to two balanced synthetic sections that both span the complete coordinate field. Two tiny synthetic SVG overlays per section live under `.trekker/images/`; neither the partition nor the layers are presented as measured tissue imagery.
- **license**: redistribution terms were not included in the received local official bundle; confirm with Trekker before public release.
- **build**: `data-raw/build_trekker_official_demo.R` → `exportFromSeurat(..., trekker_data = ..., trekker_sections = ..., trekker_images = ...)`
- **output**: `inst/extdata/examples/demo_trekker_mouse_brain_u_c.crb` + sibling `.trekker/` (~0.22 MB total)

The optional official `Trekker_Report.html` is not shipped in this size-constrained demo: that file alone is about 12.9 MB. Full user exports can include it unchanged and expose it through the Viewer download action.
