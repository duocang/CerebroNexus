...
Remove the embedded matrix before saving the smaller `.crb`; otherwise
the original matrix will still be loaded into memory.
...
Legacy `.crb` files do not contain the new backend descriptor, so
provide the H5 path explicitly when launching them:
...
This explicit override is a compatibility path. If the original Seurat
object is available, re-exporting it with
`expression_matrix_mode = "h5"` produces the portable, self-describing
pair and is less error-prone.
