...
CerebroNexus 4.5.1 closes one loose end identified by this pilot: Viewer
projection paths now retain canonical numeric cell indices through
`getExpressionRow()` and `getExpressionBlock()` instead of converting
them to barcodes and matching them back to the same columns. This
removes avoidable lookup work but does not change the larger backend
result above: BPCells single-row scans remain the dominant cost, and
storage orientation still determines the gap between single-row and
block access.
