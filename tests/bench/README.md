...
The dedicated Ren Linked transport benchmark reports three distinct endpoints. `complete_logical_ms` means the supplement has been merged; `visual_ready_ms` additionally requires every visible GPU renderer to become idle, a clonal canvas when receptor data exist, and two consecutive animation frames with identical panel geometry and opacity; `interactive_ready_ms` then waits for Shiny to become idle. Use the latter two when comparing against perceived page readiness.
