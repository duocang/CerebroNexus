...
RESULT_ROOT="${BENCH_RESULT_ROOT:-$BENCH_ROOT/result/publication-scale}"

if [ -z "${BENCH_SOURCE_CACHE:-}" ]; then
  echo "BENCH_SOURCE_CACHE is required for the publication-scale study" >&2
  exit 1
fi
if [ -z "${BENCH_STORAGE_DESCRIPTION:-}" ]; then
  echo "BENCH_STORAGE_DESCRIPTION is required for the publication-scale study" >&2
  exit 1
fi
if [ -n "${BENCH_SOURCES_ONLY:-}${BENCH_SOURCES_EXTRA:-}" ]; then
  echo "publication-scale owns the exact two-source design; source overrides are not allowed" >&2
  exit 1
fi
...
export BENCH_STUDY_ID="${BENCH_STUDY_ID:-$(date -u +%Y%m%dT%H%M%SZ)-${head_sha:0:12}-publication-scale}"
...
if [ -n "$tracked$untracked" ]; then
  echo "publication-scale requires a clean Git worktree" >&2
  exit 1
fi
