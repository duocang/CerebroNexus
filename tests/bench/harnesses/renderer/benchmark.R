...
if (is.na(count) || count < 1L || is.na(repeats) || repeats < 3L) {
  stop(
    "Usage: benchmark.R [points] [repeats] [output.csv] [screenshot.png]"
  )
}

script_arg <- grep("^--file=", commandArgs(FALSE), value = TRUE)
script <- normalizePath(sub("^--file=", "", script_arg[[1]]), mustWork = TRUE)
app_dir <- file.path(dirname(script), "app")
repo_root <- normalizePath(
  file.path(dirname(script), "..", "..", "..", ".."),
  mustWork = TRUE
)
Sys.setenv(
  CEREBRO_RENDERER_WWW = file.path(repo_root, "inst", "viewer", "www")
)
