...
test_that("client scheduler serializes file and example dispatch", {
  ...
  expect_match(client, "focusDatasetStart", fixed = TRUE)
  expect_match(client, "window.scrollTo", fixed = TRUE)
  expect_match(client, "heading.focus({ preventScroll: true })", fixed = TRUE)
