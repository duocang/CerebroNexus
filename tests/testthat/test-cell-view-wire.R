...
test_that("the browser restores compact linked-view vectors", {
  ...
  packed <- helpers$cv_wire_pack_bundle(
    list(
      ...
      groups = list(cluster = helpers$cv_group(c(0L, NA_integer_), "A", "#fff")),
      cat_extra = list(batch = helpers$cv_group(
        c(0L, 1L), c("A", "B"), c("#fff", "#000")
      )),
  ...
  writeLines(
    c(
      ...
      sprintf("eval(fs.readFileSync(%s, 'utf8'));", encodeString(wire_file, quote = "\"")),
      sprintf("const input = fs.readFileSync(%s);", encodeString(payload, quote = "\"")),
