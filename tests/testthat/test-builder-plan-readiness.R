...
test_that("spatial coordinate transforms freeze as canonical per-FOV specs", {
  local({
    builder_repo_source("preview.R")
    builder_repo_source("recommend.R")
    builder_repo_source("plan.R")
    ...
    entry <- builder_task6_entry()
    entry$dataset_profile$spatial <- list(sections = c("fov-a", "fov-b"))
    entry$settings$spatial_coordinate_transforms <- list(
      `fov-a` = list(rotation_degrees = -90, scale = 1.25)
    )
    plan <- builder_freeze_plan(list(entry), tempdir(), make_app = FALSE)

    expect_null(plan$error)
    expect_identical(
      plan$items[[1L]]$spatial_coordinate_transforms,
      list(`fov-a` = list(rotation_degrees = -90, scale = 1.25))
    )

    invalid <- entry
    invalid$settings$spatial_coordinate_transforms[["fov-a"]]$scale <- 0
    blocked <- builder_freeze_plan(list(invalid), tempdir(), make_app = FALSE)
    expect_identical(blocked$error_code, "invalid_spatial_coordinate_transform")
    expect_match(blocked$error, "strictly positive")

    unknown <- entry
    unknown$settings$spatial_coordinate_transforms$trekker <- list(
      rotation_degrees = 0,
      scale = 1
    )
    blocked <- builder_freeze_plan(list(unknown), tempdir(), make_app = FALSE)
    expect_identical(blocked$error_code, "unknown_spatial_coordinate_transform")
    expect_match(blocked$error, "unknown FOV")
  })
})
