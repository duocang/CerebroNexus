...
test_that("CSS hides projection outputs until the resize path reveals them", {
  ...
  expect_match(
    canvas_source,
    "gate.classList.add('is-sized')",
    fixed = TRUE
  )
})

test_that("Clonal UMAP observer is safe during head parsing", {
  source <- paste(
    readLines(repo_file(
      "inst",
      "viewer",
  ...
  expect_match(
    source,
    "clonalUMAPObserver.observe(document.documentElement",
    fixed = TRUE
  )
  expect_false(grepl("observe(document.body", source, fixed = TRUE))
})
