  ...
  for (spatial_name in names(spatials)) {
    ...
    images <- lapply(labels, function(label) {
      list(
        ...
        histology_image_bounds = c(xmin = 0, xmax = 100, ymin = 0, ymax = 100)
      )
    })
    names(images) <- labels
...
test_that("every copied artifact has a portable bundle target", {
  ...
  expect_error(
    build_test_app(
      ...
      spatial_images = list(
        Dataset = list(section = c(Image = reserved_image))
      )
    ),
    "portable relative path"
  )
...
test_that("private and spatial targets may share a basename", {
  ...
  suppressWarnings(build_test_app(
    ...
    app,
    spatial_images = list(
      First = list(section = c(`Tissue background` = image))
    )
  ))
...
test_that("different datasets isolate equal spatial image basenames", {
  ...
  build_test_app(
    ...
    app,
    spatial_images = list(
      First = list(section = c(`Tissue background` = first)),
      Second = list(section = c(`Tissue background` = second))
    )
  )
  config <- readRDS(file.path(app, "cerebro_config.rds"))
  expect_identical(
...
test_that("builder-owned spatial options cannot bypass formal parameters", {
  root <- withr::local_tempdir()
  crb <- write_spatial_bundle_crb(file.path(root, "source"))
  keys <- c("spatial_images", "spatial_image_settings")
...
test_that("flat spatial image manifests are rejected", {
  root <- withr::local_tempdir()
  ...
  expect_error(
    build_test_app(
      c(Dataset = single),
      file.path(root, "atomic-flat-app"),
...
test_that("one spatial image can be shared by multiple data sets", {
  ...
  build_test_app(
    ...
    app,
    spatial_images = list(
      First = list(section = c(Histology = image)),
      Second = list(section = c(Histology = image))
    )
  )
...
test_that("external spatial images render from disk without an HTTP mapping", {
  ...
  build_test_app(
    ...
    app,
    spatial_images = list(
      Dataset = list(section = c(`Tissue background` = image))
    )
  )
...
test_that("unknown spatial image datasets fail before bundling", {
  ...
  expect_error(
    build_test_app(
      ...
      app,
      spatial_images = list(
        Dataset = list(section = c(Image = matched)),
        Typo = list(section = c(Image = unmatched))
      )
    ),
    "dataset `Typo`.*cerebro_data"
  )
...
test_that("missing spatial images fail before bundling", {
  ...
  expect_error(
    build_test_app(
      ...
      app,
      spatial_images = list(
        Dataset = list(section = c(Image = missing))
      )
    ),
    "does not exist"
  )
