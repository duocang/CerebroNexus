...
test_that("browser uploads stay on HTML file transports", {
  ...
  marker <- paste(readLines(
    file.path(builder_root, "ui", "marker_import.R"),
    warn = FALSE
  ), collapse = "\n")
  enhance <- paste(readLines(
  ...
  expect_match(app, "builder_file_accept(builder_dataset_extensions())", fixed = TRUE)
  expect_match(marker, "builder_file_accept(builder_table_extensions())", fixed = TRUE)
...
test_that("macOS specs cover every directory and file picker", {
  ...
  expect_true(all(vapply(specs, function(spec) {
    grepl("on error number -128", paste(spec$args, collapse = "\n"), fixed = TRUE)
  }, logical(1))))
  expect_true(all(vapply(specs[1:2], function(spec) {
...
test_that("Linux Zenity and KDialog specs cover every picker", {
  ...
  expect_true(all(vapply(c(zenity, kdialog), function(spec) {
    identical(spec$cancel_status, 1L) &&
      identical(spec$encoding, "UTF-8")
  }, logical(1))))
  expect_true(all(vapply(zenity[1:2], function(spec) {
    "--directory" %in% spec$args
  }, logical(1))))
  expect_true("--file-filter=Builder project | *.json" %in% zenity[[3L]]$args)
  expect_true(all(vapply(kdialog[1:2], function(spec) {
