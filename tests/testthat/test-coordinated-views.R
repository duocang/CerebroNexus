...
test_that("large linked-view vectors use a lossless compact wire format", {
  ...
  header_length <- readBin(packed[seq_len(4L)], integer(), size = 4L, endian = "little")
  header <- jsonlite::fromJSON(rawToChar(packed[4L + seq_len(header_length)]), simplifyVector = FALSE)
  ...
  decode <- function(value) {
    ...
    if (identical(type, "f32")) {
      readBin(bytes, numeric(), n = value$length, size = 4L,
        endian = "little")
    } else {
      readBin(bytes, integer(), n = value$length,
        size = switch(type, i8 = 1L, i16 = 2L, 4L), signed = TRUE,
        endian = "little")
    }
  }
...
test_that("Linked views negotiates compact transport with a legacy fallback", {
  ...
  client <- paste(
    readLines(file.path(dirname(bundle_file), "..", "www", "cell_views.js"), warn = FALSE),
    collapse = "\n"
  )
