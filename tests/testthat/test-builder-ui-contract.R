...
test_that("coordinate rotation is WYSIWYG without an explicit save action", {
  ui <- paste(
    readLines(
  ...
  expect_false(grepl("save_coordinate_transform", ui, fixed = TRUE))
  expect_match(ui, "reset_coordinate_transform", fixed = TRUE)
  expect_match(
    server,
    'input[["builder_spatial_coordinate_draft"]]',
    fixed = TRUE
  )
  expect_match(server, "coordinate_session_drafts", fixed = TRUE)
  expect_match(server, "materialize_coordinate_drafts", fixed = TRUE)
  expect_false(grepl("shiny::debounce", server, fixed = TRUE))
  expect_false(grepl("coordinate_commit_pending", server, fixed = TRUE))
  expect_false(grepl("commit_coordinate_transform", server, fixed = TRUE))
  expect_false(grepl(
...
test_that("Spatial coordinate drafts flush at project, Review, and Build boundaries", {
  ...
  expect_match(build, "freeze_materialized_plan_for_output", fixed = TRUE)
})

test_that("FOV switches retain the current canvas until the next scene is ready", {
  review <- builder_asset_text("server", "review.R")
  server <- builder_asset_text("spatial_alignment_server.R")
  ...
  request_preview <- regmatches(
    server,
    regexpr(
      "request_preview <- function[\\s\\S]*?invisible\\(queued\\)\\n  }",
      server,
      perl = TRUE
    )
  )
  expect_false(grepl("alignment_preview(NULL)", request_preview, fixed = TRUE))
  expect_match(
    server,
    "!identical(preview$section$id, section)",
    fixed = TRUE
  )
  set_scene <- regmatches(
    canvas,
    regexpr("function setScene[\\s\\S]*?\\n  }", canvas, perl = TRUE)
  )
  expect_false(grepl("if (viewChanged) clear();", set_scene, fixed = TRUE))
  expect_match(canvas, "controlsHeld: false", fixed = TRUE)
  expect_match(
    canvas,
    'event.target.id === "enhance-active_section"',
    fixed = TRUE
  )
  expect_match(canvas, "if (state.controlsHeld) return false", fixed = TRUE)
  expect_match(canvas, "state.controlsHeld = false", fixed = TRUE)
})

test_that("spatial sidebar does not stretch empty space below the FOV picker", {
  css <- builder_asset_text("www", "builder.features.css")
  expect_match(css, ".spatial-alignment-sidebar {", fixed = TRUE)
  expect_match(css, "align-content: start", fixed = TRUE)
})
...
test_that("dataset rail separates lifecycle colours from interaction states", {
  ...
  expect_match(components, ".ds-ready-stamp", fixed = TRUE)
  expect_match(
    components,
    '.ds.is-active[data-load-state="reading"]',
    fixed = TRUE
  )
  expect_match(
    components,
    "background: var(--builder-rail-progress-bg)",
    fixed = TRUE
  )
  expect_match(
    components,
    "background: var(--builder-rail-progress-fg)",
    fixed = TRUE
  )
  expect_match(layout, ".ds-pick:focus-visible", fixed = TRUE)
  expect_match(layout, "translateY(-1px)", fixed = TRUE)
  expect_match(components, "prefers-reduced-motion: reduce", fixed = TRUE)
  expect_false(grepl(
    ".ds-picker .ds:not(.is-active):has(.ds-pick:hover)::before",
    layout,
    fixed = TRUE
  ))
})
...
test_that("Build gives immediate visible progress before server preparation", {
  js <- builder_asset_text("www", "builder.js")
  css <- builder_asset_text("www", "builder.features.css")
  ...
  expect_false(grepl("document.documentElement.scrollHeight", js, fixed = TRUE))
  expect_match(css, ".builder-build-waiting .spinner", fixed = TRUE)
  expect_match(css, "var(--c-success)", fixed = TRUE)
  expect_match(css, "#build_stage_status_content.recalculating", fixed = TRUE)
  expect_false(grepl("is-build-focus-target", css, fixed = TRUE))
})
...
test_that("dataset rail uses one selected state and one soft-danger action", {
  ...
  expect_match(components, ".builder-client-import-queue > .ds", fixed = TRUE)
  expect_match(
    components,
...
test_that("pipeline visualization reflects the current step", {
  ...
  expect_match(js, 'state === "complete"', fixed = TRUE)
  expect_match(js, 'classList.toggle("is-terminal"', fixed = TRUE)
  expect_match(css, ".pipeline-step.is-terminal", fixed = TRUE)
