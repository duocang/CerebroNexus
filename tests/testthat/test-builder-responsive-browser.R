...
builder_responsive_geometry <- function(app) {
  app$get_js(paste0(
    ...
    "previewColumnCount: plotRects.length === 2 && Math.abs(plotRects[0].top - plotRects[1].top) <= 2 ? 2 : 1,",
...
test_that("Builder preserves responsive geometry before Build", {
  ...
  app$wait_for_js(
    paste0(
      ...
      "document.querySelectorAll('.spatial-alignment-plot-frame').length === 1"
    ),
    timeout = 60000
  )
  app$wait_for_idle(timeout = 30000)
  ...
  for (viewport in list(
    ...
    expect_identical(geometry$previewColumnCount, 1L)
    expect_identical(
