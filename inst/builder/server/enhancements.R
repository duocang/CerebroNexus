...
schedule_enhance_table_files <- function(
  ...
  builder_async_then(
    task,
    session,
    on_fulfilled = function(inventories) finish(inventories),
    on_rejected = function(error) finish(failed = TRUE)
  )
  session$onSessionEnded(function() builder_async_cancel(task))
  invisible(TRUE)
}
            ...
            session$sendCustomMessage(
              ...
              list(
                ...
                text = paste0(
                  ...
                  "Click Add tables again to upload through the browser."
                )
              )
            )
          } else {
...
observeEvent(
  input$builder_cancel_file_picker,
  {
    ...
    if (is.list(request) && identical(request$kind, "tables")) {
      builder_cancel_native_picker("extra-material-tables")
      native_table_picker_active <<- FALSE
    }
  },
  ignoreInit = TRUE
