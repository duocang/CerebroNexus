...
.builder_release_read_record <- function(
  ...
  if (length(missing) || any(wrong_type)) {
    .builder_release_record_error("does not match its recorded members.")
  }
  foreign <- setdiff(actual_paths, member_paths)
...
builder_release_state <- function(target, exact_record = TRUE) {
  identity <- builder_release_identity(target)
  record <- if (isTRUE(identity$exists)) {
    .builder_release_read_record(target, identity, exact = exact_record)
  } else {
    NULL
  }
...
builder_publish_release <- function(
  ...
  current_state <- builder_release_state(handle$target, exact_record = FALSE)
