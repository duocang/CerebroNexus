...
builder_project_save_actions_dialog <- function(
  checked,
  reusable,
  source_syncing = FALSE
) {
  remaining <- checked - reusable
  primary <- if (isTRUE(source_syncing)) {
    "Project settings are saved. Source files are still being copied."
  } else {
    "Your datasets and current settings are saved."
  }
  secondary <- if (isTRUE(source_syncing)) {
    "Keep this page open until Builder reports that the project is fully saved."
  } else if (remaining > 0L) {
    paste0(
      remaining,
      " checked dataset",
      if (remaining == 1L) " has" else "s have",
      " no reusable CRB yet."
    )
  } else {
    "You can reopen this project and continue later."
  }
  modalDialog(
    title = if (isTRUE(source_syncing)) {
    ...
    builder_project_dialog_content(
      primary,
      secondary,
      if (isTRUE(source_syncing)) "copy" else "check"
    ),
    footer = tagList(
      modalButton("Done"),
      if (checked > reusable) {
