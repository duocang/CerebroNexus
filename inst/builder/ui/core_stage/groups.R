...
builder_metadata_policy_status <- function(policy, id) {
  ...
  list(
    ...
    recommended = isTRUE(
      record$recommended_retain_in_crb %||% record$retain_in_crb
    ),
    supported = if (
      is.list(record) &&
...
builder_group_catalog_model <- function(model) {
  ...
  items <- lapply(seq_along(ids), function(index) {
    ...
    recommendation_status <- builder_metadata_policy_status(
      model$metadata_recommendation %||% model$metadata_policy %||% list(),
      id
    )
    ...
    list(
      ...
      recommended_retained = isTRUE(recommendation_status$retained),
      included = selected,
