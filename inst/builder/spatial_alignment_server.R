...
builder_spatial_alignment_server <- function(
  ...
  collection_for <- function(entry) {
    ...
    if (
      ...
      cached <- builder_image_collection_normalize(
        image_collection_cache$images
      )
      merged <- stored
      known_sections <- image_collection_cache$known_sections %||%
        names(cached)
      for (section in known_sections) {
        merged[[section]] <- cached[[section]]
      }
      return(builder_image_collection_normalize(merged))
    }
    stored
  }
  ...
  show_remove_image <- function() {
    ...
    section <- active_section()
    label <- active_image()
    if (is.null(label) && !is.null(entry) && !is.null(section)) {
      labels <- image_labels_for(entry, section)
      label <- if (length(labels)) labels[[1L]] else NULL
    }
  ...
  shiny::observeEvent(input[["enhance-remove_image"]], show_remove_image())
  ...
  list(
    ...
    draft = draft,
    coordinate_draft = coordinate_draft,
