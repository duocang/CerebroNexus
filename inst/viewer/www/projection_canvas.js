...
(function () {
  ...
  function ensure(id) {
    ...
    toolbar.innerHTML = '<button type="button" data-mode="select" class="is-active" title="Box select">□</button><button type="button" data-mode="lasso" title="Lasso select">△</button><button type="button" data-mode="pan" title="Pan">✥</button><button type="button" data-action="reset" title="Reset view">⌂</button><button type="button" data-action="download" title="Download PNG">⇩</button>';
    ...
    state = {
      ...
      radius: 2.5, opacity: .85, raf: null, mode: 'select', toolbar: toolbar
    };
    plots.set(id, state);
    bind(state);
