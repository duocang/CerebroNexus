...
var focusPanel = null;
  ...
  // Builder appearance seeds each physical space until the user deliberately
  // changes the shared workspace control.
  ...
  function pointSizeOf(p) {
    ...
    if (!pointSizeEdited && isFinite(configured) && configured >= 0 && configured <= 20) {
      return configured;
    }
    return ps;
  }
  function pointOpacityOf(p) {
    ...
    if (!pointOpacityEdited && isFinite(configured) && configured >= 0 && configured <= 1) {
      return configured;
    }
    return pointOpacity;
  }
  ...
  // State identity is not the visible label. A physical FOV can legitimately
  // be named "Trekker", so modality kind + stable space id must namespace its
  // choice and calibration away from the direct Trekker background.
  ...
  function onData(bundle) {
    ...
    focusPanel = null;
    groupFilter = {};
    // Builder defaults seed only a genuinely new data set. A same-dataset
    // refresh after recolouring must preserve deliberate Viewer overrides.
    if (dataChanged) {
      ...
      var configuredPct = Number(D.default_percentage_cells_to_show);
      pctShow = D.default_percentage_cells_to_show != null && isFinite(configuredPct)
        ? Math.max(10, Math.min(100, configuredPct)) : 100;
      rebuildPctMask();
