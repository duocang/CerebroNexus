...
cv_wire_pack_bundle <- function(
  ...
  bundle$cells <- if (isTRUE(include_cells)) pack(bundle$cells, "json") else NULL
...
cv_wire_pack_message <- function(message, min_length = 4096L) {
  ...
  pack <- function(values, type) {
    bytes <- if (identical(type, "json")) {
      ...
      size <- switch(type, i8 = 1L, i16 = 2L, i32 = 4L,
        f32 = 4L, f64 = 8L)
      writeBin(
    ...
    alignment <- switch(type, i8 = 1L, i16 = 2L, i32 = 4L,
      f32 = 4L, f64 = 8L, 1L)
    offset <- data_size + ((alignment - data_size %% alignment) %% alignment)
    chunks[[length(chunks) + 1L]] <<- list(offset = offset, bytes = bytes)
    data_size <<- offset + length(bytes)
  ...
  walk <- function(value, field = NULL) {
    ...
    if (!is.atomic(value) || length(value) <= 1L || length(value) < min_length) {
      return(value)
    }
...
cv_build_bundle <- function(crb) {
  ...
  if (length(projections)) {
    ...
    ## from there. Keeping another x/y/z copy in `spaces` doubled the largest
    ## part of the million-cell wire payload.
