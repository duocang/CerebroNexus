...
configured_spatial_images <- function(
  ...
  if (
    !is.list(options) ||
      !valid_identity(dataset) ||
      !valid_identity(spatial_name)
  ) {
    return(list())
  }
  manifest <- options[["spatial_images"]]
  if (
    is.null(manifest) ||
      is.null(names(manifest)) ||
      !dataset %in% names(manifest)
  ) {
    return(list())
  }
  dataset_images <- manifest[[dataset]]
  if (
    !is.list(dataset_images) ||
      is.null(names(dataset_images)) ||
      !spatial_name %in% names(dataset_images)
  ) {
    return(list())
  }
  leaf <- dataset_images[[spatial_name]]
...
resolve_spatial_image_setting <- function(
  ...
  if (
    !is.list(options) ||
      !valid_identity(dataset) ||
      !valid_identity(spatial_name) ||
      !valid_identity(image_label) ||
      !valid_identity(setting)
  ) {
    return(fallback)
  }
  settings <- options[["spatial_image_settings"]]
  value <- NULL
  if (
    is.list(settings) &&
      dataset %in% names(settings) &&
      is.list(settings[[dataset]]) &&
      spatial_name %in% names(settings[[dataset]]) &&
      is.list(settings[[dataset]][[spatial_name]]) &&
      image_label %in% names(settings[[dataset]][[spatial_name]]) &&
      is.list(settings[[dataset]][[spatial_name]][[image_label]])
  ) {
    value <- settings[[dataset]][[spatial_name]][[image_label]][[setting]]
  }
