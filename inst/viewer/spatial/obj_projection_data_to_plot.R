...
spatial_projection_data_to_plot_raw <- reactive({
  ...
  if (plot_parameters$plot_type == "Co-expression (RGB)") {
    ...
    selected_genes <- unique(unlist(coexpr_genes, use.names = FALSE))
    selected_genes <- selected_genes[
      !is.na(selected_genes) &
        nzchar(selected_genes) &
        selected_genes %in% getGeneNames()
    ]
    expression_data <- if (length(selected_genes)) {
      data_set()$getExpressionMatrix(
        cells = cells_to_extract,
