...
.hydrateThinCrb <- function(obj, crb_path, sidecar, schema) {
  ...
  if (
    ...
    stop(
      "The compact CRB and BPCells sidecar have incompatible cell metadata.",
      call. = FALSE
    )
  }
  ...
  if (length(missing_projections)) {
    stop(
      "The compact CRB is missing projection '",
  ...
  for (name in schema$projection_rownames) {
    ...
    if (!is.data.frame(projection) || nrow(projection) != length(cells)) {
      stop(
        "Projection '",
        name,
        "' does not match the compact CRB cell index.",
        call. = FALSE
      )
    }
...
.attachExternalExpression <- function(
  ...
  if (identical(be$mode, "embedded")) {
    if (!is.null(crb_schema)) {
      stop(
        "A compact CRB requires a BPCells expression backend.",
        call. = FALSE
      )
    }
    return(obj)
