...
expression_projection_expression_levels <- reactive({
  ...
  ## The useful empty state is the projection itself. Do not open a progress
  ## overlay or touch the expression matrix before a valid gene is selected.
  if (length(genes_present) == 0) {
    return(rep(0, length(cells_to_show)))
  }
